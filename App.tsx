...
const App: React.FC = () => {
  ...
  const startGame = (config: GameConfig) => {
    ...
    const newPlayers: Player[] = config.playerNames.map((playerName, i) => {
      ...
      if (chaosModeEnabled) {
        // En modo caos, cada uno recibe una palabra distinta del pool
        // Si el pool es más pequeño que los jugadores, repetimos o avisamos (aquí usamos el índice con módulo)
              ...
              <div className="space-y-6 max-h-[80vh] overflow-y-auto pr-2 custom-scrollbar">
                ...
                <div className="space-y-4">
                  ...
                  >
                    ...
                    <div className={`w-14 h-8 rounded-full relative transition-colors duration-300 ${manualImpostorEnabled ? 'bg-indigo-600' : 'bg-slate-700'}`}>
                  ...
                  >
                    ...
                    <div className={`w-14 h-8 rounded-full relative transition-colors duration-300 ${chaosModeEnabled ? 'bg-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.4)]' : 'bg-slate-700'}`}>
                  ...
                  {chaosModeEnabled && (
                    <div className="bg-amber-500/10 border border-amber-500/20 p-3 rounded-xl animate-fade-in">
                      <p className="text-[10px] text-amber-200/70 font-medium leading-relaxed">
                        En este modo, **todos** los jugadores recibirán una palabra diferente de la categoría seleccionada. Los roles (Impostor/Civil) se mantienen solo como etiqueta.
                      </p>
                    </div>
                  )}
                </div>
